...
while running:
    ...
    attack_text = font.render("Appuyez sur ESPACE pour attaquer", True, BLACK)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if player_turn:
                ...
                print(f"{player_pokemon.name} inflige {damage} dégâts à {enemy_pokemon.name}!")
                player_turn = False

            elif enemy_pokemon.hp > 0:
                pygame.time.delay(1000)
                damage = enemy_pokemon.attack_pokemon(player_pokemon)
                print(f"{enemy_pokemon.name} inflige {damage} dégâts à {player_pokemon.name}!")
                player_turn = True
